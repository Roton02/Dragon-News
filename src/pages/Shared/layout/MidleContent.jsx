...
const MidleContent = ({news}) => {
    // console.log(news);
    const {title,image_url,img,total_view,details,_id} =news;
    return (
        <div>
	...
	<div>
		...
		<p className="text-sm dark:text-gray-600">
            {details.length > 200 ? <>{details.slice(0,200)} <Link 
			className="text-blue-400" to={`news/${_id}`}> Read More .... </Link> </> : details}</p>
	</div>
